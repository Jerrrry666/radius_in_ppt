...
If you also work on rounded rectangles in PowerPoint on Mac, feel free to try it and submit issues / PRs / suggestions — any feedback is welcome 🙏

A custom **"RadiusInPpt"** tab in PowerPoint's ribbon (a la iSlide) that lets you set the corner radius of rounded rectangles precisely in **centimeters** or **percentages**, with absolute-value locking, anti-misclick, a style brush, 5-step history, and the **v1.2 Layout Mode** (rows × cols grid + padding/gutter sliders + R-corner coupling).
...
- **AGENTS.md §1.0 new rule** — AI commit/push permission limit
...
Or just double-click `RadiusInPpt.app` — it auto-starts the server and registers the add-in.
...
> `RadiusInPpt.app` does this automatically on launch.

### 3. Load into PowerPoint

1. **Fully quit PowerPoint** (`Cmd+Q` — don't just close the window)
2. **Reopen** PowerPoint
3. The **"Home"** tab → **"Add-ins"** button
4. Pick **"RadiusInPpt"** in the popup
5. The **"RadiusInPpt"** tab appears in the ribbon
...
2. The **"RadiusInPpt"** tab → **"Adjust R-corner"** button
3. The Dialog opens:
   - Top status card: current selection info + current R (for multi-select, shows min~max)
   - Type a cm value in the input, e.g. `0.3` → click **"Apply R"** or press Enter
   - Click **"Fix R by value"**: locks R to absolute cm on all selected roundRects; click again → all unlock
   - "Re-apply lock (current selection)": after resizing, use this to re-apply the stored absolute R

### v1.2 Layout Mode

1. **Draw your nesting**: 1 large rounded rectangle (parent) + N small ones (children), positions / sizes arbitrary
2. **Select all** (parent + all children) → **"RadiusInPpt"** tab → **"Adjust R-corner"**
3. Pick 1 as **parent** (click the parent row in the shape list — "Build layout" button appears)
4. Fill **rows × cols** (e.g. `2 × 3`) → pick **child list** → **"Build layout"**
...
**Smoke test (in-PPT)**: Task pane → click the "🧪 Driver smoke test" button → 14/14 passing = driver verified.
